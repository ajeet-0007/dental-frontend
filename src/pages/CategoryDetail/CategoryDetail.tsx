...
export default function CategoryDetail() {
  ...
  const { data: details } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get(`/categories/slug/${slug}/details`);
      return response.data;
    },
  ...
  const { data: featuredData } = useQuery({
    queryKey: ["products", "category", slug, "featured"],
    queryFn: async () => {
      const response = await api.get("/products", {
        params: { categories: slug, isFeatured: true, limit: 12 },
      });
      return response.data;
    },
    enabled: !!slug && !isError,
  ...
  const gradientClass =
    INITIAL_COLORS[Math.abs(((category?.id ?? 1) - 1) % INITIAL_COLORS.length)];

  const scrollToProducts = () => {
    document
      .getElementById("shop-the-category")
  ...
  return (
    <>
                  ...
                  >
                    <span
                      className={`w-6 h-6 rounded-lg bg-gradient-to-br ${
                        INITIAL_COLORS[
                          Math.abs(
                            ((child.id ?? 1) - 1) % INITIAL_COLORS.length
                          )
                        ]
      ...
      {/* Shop the category */}
      <div id="shop-the-category" className="scroll-mt-4">
        <Products
          categorySlug={slug}
          seoTitle={entityName}
